...
### 方案可执行性审查（逐项 Yes / No，No → DA-CRITICAL）

DA 对研究方法章节逐项判定，每项只能回答 Yes 或 No，不得模糊：

| # | 检查项 | 判定 | 说明 |
|---|--------|------|------|
| 1 | 每个核心技术决策是否已落实到**具体选项**（而非"先进方法"/"某种技术"等占位符）？ | Y/N | |
| 2 | 每个已选定的具体选项，是否说明了**为何选它而非备选方案**？ | Y/N | |
| 3 | 每个实验是否有**完整执行条件**：具体数据集名称 + 具体评估指标 + 具体 baseline 列表（不得只写"主流方法"）？ | Y/N | |
| 4 | 尚未确定的决策，是否明确说明了**用什么实验/方法来确定它**（不得只写"待调参"/"后续确定"）？ | Y/N | |
| 5 | 研究方法中是否存在**无法从报告内容推断如何实现**的步骤（即读完仍不知道怎么做）？ | Y/N（存在=N） | **判定为 N 时必须**：① 逐字引用报告中不可实现的原句；② 说明缺少什么信息才能实现；③ 给出修改方向（补充具体操作 / 引用已有实现 / 标注"由实验X确定"并说明实验设计）|
| 6 | 实施路径的**第一个具体步骤**是否已描述（具体操作，不是"调研背景"）？ | Y/N | |

任意一项为 No → 对应内容列为 DA-CRITICAL，必须在报告中补充具体说明。
...
改进内容 → 分类路由：
  ...
  · 文献缺失问题 → 返回 S3 补充精读
  · 方案可执行性（DA 第 1-5 项任意 No）→ 二次判断：
      - 研究计划本身未确定（S5 输出中该决策仍为空洞/待定）→ 返回 S5 补充计划，重新过 S5 自检门，再重跑 S6 + S7 + S9
      - 计划已有具体内容但报告未如实写出 → 返回 S6 补充研究方法章节对应段落，重新过 S6 报告完整门，再重跑 S9
