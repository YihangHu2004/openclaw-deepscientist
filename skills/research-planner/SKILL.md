...
### 核心方案草案
具体说明**怎么做**——工具/算法/流程的选择及理由。每个关键决策落实到具体选项，说明为何选它而非备选。禁止"将采用先进技术"等空洞表述。

### 实施路径
分步骤描述如何从零推进到可验证结果，每步有明确输入/输出/工具。

### 可行性评估
- 主要障碍（≥ 2 条）及应对方案
- 执行难度(1-5) / 数据获取方式 / 资源需求（若涉及计算，给出估算依据）
...
**方案可执行性自检（逐项 Yes/No，任意 No → FAIL，不得进入 S6）**：

| # | 检查项 | 判定 |
|---|--------|------|
| 1 | 每个核心技术决策是否已落实到**具体选项**（无"先进方法"/"某种技术"等占位符）？ | Y/N |
| 2 | 每个已选定的具体选项，是否说明了**为何选它而非备选方案**？ | Y/N |
| 3 | 每个实验是否有**完整执行条件**：具体数据集名称 + 具体评估指标 + 具体 baseline（不得只写"主流方法"）？ | Y/N |
| 4 | 尚未确定的决策，是否说明了**用什么实验/方法来确定它**（不得只写"待调参"/"后续确定"）？ | Y/N |
| 5 | 实施路径的**第一个具体步骤**是否已描述（具体操作，不是"调研背景"）？ | Y/N |

判定为 No 时，**必须**：① 引用计划中不具体的原句；② 说明缺少什么；③ 补充具体内容后重新自检。
