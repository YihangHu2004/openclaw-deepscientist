...
**四、研究方法** 子节强制内容：
- **4.1 整体架构设计**：必须包含模块划分图（文字描述或 ASCII 图）、数据流向、各模块职责。不得只写"本研究提出一种新方法"等空洞表述。
- **4.2 核心模块与模型选型**：每个核心模块必须明确说明——选用哪个具体模型/算法（如 BERT-base / ResNet-50 / Transformer decoder），选择理由（性能/效率/可复现性），以及与备选方案的对比。每项选型决策必须有 EV 支撑或明确标注 `[MATERIAL GAP: 未找到对比实验依据]`。
- **4.3 技术可行性分析**：计算资源需求（显存/训练时间估算）、关键技术风险（≥2 条）及应对方案。
...
- **"选用 X 模型/架构"** → 必须有 EV 说明选择依据（性能数据 / 参数量 / 已有复现结果），或标注 `[MATERIAL GAP: 选型依据待验证]`
- **模型超参数或架构细节**（层数、隐层维度、注意力头数等）→ 必须来自参考文献 EV 或明确标注为"待消融实验确定"
- **"计算资源需求为 X"** → 必须有来源（文献报告的训练时间 / 显存），不得凭空估算
...
| **可行性方案完整** | 研究方法章节必须包含 4.1 架构设计、4.2 模型选型、4.3 技术可行性三个子节，任一缺失 → FAIL |
| **模型选型有据** | 每个核心模型/算法选择必须有 EV 支撑或 `[MATERIAL GAP]` 标注，不得只出现模型名称而无选型理由 |
| **技术风险** | 4.3 节中技术风险 ≥ 2 条，每条附应对方案 |
