...
获取 arXiv HTML 后，**额外**用 BeautifulSoup 提取所有 `<table>` 元素，转为 Markdown 表格，追加到正文末尾：
```python
...
```
同样提取 `<figcaption>` 图注，记录图表描述（不需全图，仅标题 + 位置说明）。
表格信息优先用于 Skill 4 的方法对比表，EV 记录中引用表格数据时 claim_location 注明"Table N"。
...
- 截取**逐字原文**段落（不改写）
- 在笔记中标注 EV-xxx
- 写入 evidence.json（ev_id 自增，按以下规则设置 source_type 和 confidence）：
