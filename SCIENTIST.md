...
- **门控数字是下限，不是目标**：任何验收门中的数量要求（如"≥10 条 EV"）是最低通过线，不是工作量上限。到达下限后必须自问"内容是否真正完整？"而非直接停止。survey 类论文、长篇综述等内容密集型材料应主动超过下限。
...
- 每次 web_fetch 调用 arXiv Export API（`export.arxiv.org`）后，**必须等待 ≥ 5 秒**再发下一个请求
- **绝对禁止**在同一 turn 内并行发起多个 arXiv 请求
- 收到 429：线性退避重试，最多 5 次（30 → 60 → 90 → 120 → 150 秒）
- 5 次重试全部失败 → 按以下降级链切换，**不得停止搜索任务**：
  1. Semantic Scholar MCP `search_papers()` 提取 arXiv ID → `web_fetch arxiv.org/abs/{id}`
  2. `web_search "arxiv {关键词}"` → 逐条 `web_fetch arxiv.org/abs/{id}`（间隔 ≥ 5s）
  3. `web_fetch "https://arxiv.org/search/?query={关键词}&searchtype=all"`（页面路径，非 API）
- **注意**：`arxiv.org/abs/`、`arxiv.org/html/`、`arxiv.org/search/` 页面路径与 API 限速独立，API 429 不影响直接访问论文页面
